...
CẢNH BÁO CHỚP NHÁY: Tuyệt đối KHÔNG viết gsap.to() hoặc gsap.fromTo() target sceneEl với opacity: 0 (exit animation cho scene container).
Việc này sẽ ẩn luôn background layer và gây chớp nháy đen giữa các cảnh. Chỉ được fade .scene-text-card, .scene-line-card hoặc .scene-content bên trong.
...
for (var i = 0; i < SCENES_DATA.length; i++) {
  ...
  // G. TUYỆT ĐỐI KHÔNG viết exit animation (opacity: 0) target sceneEl — sẽ gây chớp nháy nền đen!
  // Chỉ được phép fade nội dung bên trong (.scene-text-card, .scene-line-card, .scene-content).
  // Crossfade nền sẽ tự nhiên nhờ scene kế đè lên opacity.
...
for (var i = 0; i < SCENES_DATA.length; i++) {
  ...
  // H. TUYỆT ĐỐI KHÔNG viết exit animation (opacity: 0) target sceneEl — sẽ gây chớp nháy nền đen!
  // Chỉ được phép fade nội dung bên trong (.scene-text-card, .scene-line-card, .scene-content).
  // Crossfade nền sẽ tự nhiên nhờ scene kế đè lên opacity.
