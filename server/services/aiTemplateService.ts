...
`;
  try {
    ...
    sanitizedHtml = sanitizedHtml.split('var(--body-font)').join("'plus jakarta sans'");
    ...
    sanitizedHtml = sanitizedHtml.split('var(--font-family-body)').join("'plus jakarta sans'");
    ...
    // 3. Chuẩn hóa triệt để bất kỳ phông chữ nào không nằm trong danh sách hỗ trợ của HyperFrames (Banned fonts will be cleaned)
    const ALLOWED_FONTS = [
    ...
    sanitizedHtml = sanitizedHtml.replace(/font-family\s*:\s*([^;}]+)/gi, (match, fontVal) => {
      ...
      // Nếu chứa phông lạ, tự động quy về phông an toàn
